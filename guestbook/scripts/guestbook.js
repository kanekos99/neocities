(function () {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        errorContainer.innerHTML = "The provided answer to the challenge question is invalid!";
      } else {
  ...
  function guestbooks___populateQuestionChallenge() {
    ...
    const challengeHint = "Just type yes";
  ...
  function guestbooks___loadMessages(reset) {
    ...
    var apiUrl =
      "https://guestbooks.meadow.cafe/api/v2/get-guestbook-messages/897?page=" + currentPage + "&limit=" + pageSize;
    fetch(apiUrl)
